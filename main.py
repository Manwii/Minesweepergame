...
'''  numpy manages arrays and numerical computations - creating and manipulating arrays (for the game grid,
     mine locations, counts, flags, etc.), random shuffling (for mine placement), and logical operations.

     itertools.products creates Cartesian products, useful for generating combination of grid indices. 
     Useful for iterating over adjacent squares around a given square in the grid, particulary when
    revealing multiple squares or checking flagged mines

    scipy.signal.convolve2d is used for 2d convolution. used to calculate the number of mines adjacent
    to each square of the grid

    matplotlib.pyplot creates figures, plotting game grids, mines, and flags. and handling graphical aspects
    including rendering the game window and reposnding to mouse events

    matplotlib.patches.regularpolygon creates regular polygon shapes, specifically for drawing the square tiles
